...
- If the user asks only for Code Connect mappings, first verify a Code Connect-capable tool or skill is installed; this bundle has no dedicated Code Connect skill.
- If the user asks to author reusable agent rules (`CLAUDE.md`/`AGENTS.md`), treat that as a separate project-instructions task and follow the repository's own guidance; do not route to a missing skill.
